...
export function TeammateDialog({ teammate, onSave, onClose }: { teammate?: Teammate | null; onSave: (value: { name: string; persona: string; color: string; emoji: string }) => void; onClose: () => void }) {
  ...
  useEffect(() => { setName(teammate?.name || ''); setPersona(teammate?.persona || ''); setColor(teammate?.color || colors[0]); setEmoji(teammate?.emoji || '✦'); }, [teammate]);
  ...
  return <div className="modal-backdrop"><form className="teammate-dialog" role="dialog" aria-modal="true" aria-labelledby="teammate-title" onSubmit={event => { event.preventDefault(); if (name.trim()) onSave({ name: name.trim(), persona: persona.trim(), color, emoji }); }}>
