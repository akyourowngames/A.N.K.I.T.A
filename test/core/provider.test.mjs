...
import { fetchWithRetry } from '../../src/core/net.mjs';

const provider = await import('../../src/core/provider.mjs').catch(() => ({}));
const temp = fs.mkdtempSync(path.join(os.tmpdir(), 'provider-test-'));
process.env.CONFIG_DIR = path.join(temp, 'config');
const { loadConfig, CONFIG_DIR } = await import('../../src/core/config.mjs');
