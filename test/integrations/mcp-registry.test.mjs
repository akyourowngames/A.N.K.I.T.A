...
const {
  ...
  shortName,
  isLatest,
} = await import('../../src/integrations/mcp-registry.mjs');
...
test('install records the server and refuses to run it', async () => {
  const manage = await import('../../tools/mcp/mcp-manage.mjs');
  const { McpStore } = await import('../../src/integrations/mcp-store.mjs');
  ...
  const { commandHash } = await import('../../src/integrations/mcp-store.mjs');
