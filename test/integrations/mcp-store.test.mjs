...
const { McpStore, commandHash, describeServer, MCP_VERSION } = await import('../../src/integrations/mcp-store.mjs');
const { McpManager } = await import('../../src/integrations/mcp-manager.mjs');
const { Daemon } = await import('../../src/automation/daemon.mjs');
const manage = await import('../../tools/mcp/mcp-manage.mjs');
const { MCP_FILE } = await import('../../src/core/config.mjs');
...
test('the /mcp command and the mcp_manage tool word things identically', async () => {
  ...
  const { enableMessage, disableMessage } = await import('../../src/integrations/mcp-store.mjs');
  const store = new McpStore(MCP_FILE).load();
  for (const s of [...store.servers]) store.remove(s.id);
