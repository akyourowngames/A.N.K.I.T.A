...
class SemanticToolManifest:
    ...
    def shortlist(self, user_message: str, tools: list[dict[str, Any]]) -> SemanticManifestResult:
        if not tools or self.limit <= 0 or self.embedder is None:
            return SemanticManifestResult(tools=tools, best_score=1.0 if tools else 0.0, used_semantic_ranking=False)
        ...
        try:
            ...
            return SemanticManifestResult(tools=tools, best_score=1.0, used_semantic_ranking=False)

        ranked = sorted(zip(scores, tools, strict=False), key=lambda item: item[0], reverse=True)
        best_score = ranked[0][0] if ranked else 0.0
        if best_score < self.min_score:
            return SemanticManifestResult(tools=[], best_score=best_score, used_semantic_ranking=True)
        selected = [tool for score, tool in ranked[: self.limit] if score >= self.min_score]
        return SemanticManifestResult(tools=selected, best_score=best_score, used_semantic_ranking=True)
