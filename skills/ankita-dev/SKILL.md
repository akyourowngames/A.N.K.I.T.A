...
2. Map the path of the behavior you are changing:
   ...
   - `src/cli.mjs` handles the REPL and slash commands.
   - `src/agent.mjs` builds prompts and runs the model and tool loop.
...
7. Run the focused test, then `node --test "test/*.test.mjs"` before reporting completion.
