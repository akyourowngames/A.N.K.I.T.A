...
- **Option A — Azure Trusted Signing** (recommended): ~US$10/month, no hardware
  token, works well in CI. This is what `desktop/packaging/electron-builder.signing.yml` targets.
...
`AZURE_CLIENT_ID` and builds with `--config desktop/packaging/electron-builder.signing.yml`, which
...
npx electron-builder --win --publish never --config desktop/packaging/electron-builder.signing.yml
```
...
Local unsigned builds are unaffected: `desktop/packaging/electron-builder.yml` alone never signs.
