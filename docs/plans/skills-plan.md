...
Status: implemented. This records the original chat REPL plan; Desktop interactive chats now use the same built-in skills.
Scope lock: `suggested-tools` is a hint, never enforced. Standalone daemon agents remain outside this feature; Desktop Telegram messages share the enabled teammate chat.
