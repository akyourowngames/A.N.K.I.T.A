 ...
 *
 * Usage:
 *   node scripts/bench-latency.mjs                       # configured model, 3 runs
 *   node scripts/bench-latency.mjs --runs 5
 *   node scripts/bench-latency.mjs --models "openai/gpt-4.1,deepseek/deepseek-v4-flash"
 *   node scripts/bench-latency.mjs --tools               # add a tool-round prompt
 *   node scripts/bench-latency.mjs --prompt "summarise this repo"
 *   node scripts/bench-latency.mjs --json                # machine-readable
 *
...
import { readAuth, CopilotClient } from "../src/auth.mjs";
import { CompatibleClient, pickModel, resolveProvider } from "../src/provider.mjs";
...
async function main() {
  ...
  if (args.help) {
    console.log("usage: node scripts/bench-latency.mjs [--runs N] [--warmup N] [--models a,b] [--tools] [--prompt \"...\"] [--json]");
    return;
  }
