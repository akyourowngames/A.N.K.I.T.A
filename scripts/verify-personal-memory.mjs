...
// Run: node scripts/verify-personal-memory.mjs --live
...
try {
  const { loadConfig, PROFILE_FILE, PROJECTS_FILE } = await import('../src/config.mjs');
  const { Agent } = await import('../src/agent.mjs');
  const { CopilotClient } = await import('../src/auth.mjs');
  const { CompatibleClient, pickModel } = await import('../src/provider.mjs');
  const { ProfileStore } = await import('../src/profile.mjs');
  const { ProjectStore } = await import('../src/projects.mjs');
  const { recordTurn } = await import('../src/sessions.mjs');
  const { MemoryConsolidator } = await import('../src/consolidate.mjs');
  const { extractMemory } = await import('../src/memory-extract.mjs');
  const { run: recall } = await import('../tools/recall.mjs');
