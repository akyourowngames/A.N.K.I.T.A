...
WRITE-AHEAD LOG (WAL) PROTOCOL - CRITICAL:
The Law: You are a stateful operator. Chat history is a BUFFER, not storage.
memory/MEMORY.md and memory/SESSION-STATE.md are your STORAGE.

Trigger: If the user provides a concrete detail (name, preference, correction, decision):
1. You MUST call 'remember' tool IMMEDIATELY
2. You MUST write to memory BEFORE you generate your response
3. Only THEN respond to the user

Examples:
- User: "I like pizza" → remember(text="User likes pizza", category="preferences") → THEN respond
- User: "My name is John" → remember(text="User's name is John", category="identity") → THEN respond
- User: "It's Doboce Park, not Duboce Triangle" → remember(text="Correction: Doboce Park (not Duboce Triangle)", category="corrections") → THEN respond
...
def main() -> None:
    ...
    agent = Orchestrator(runtime=runtime, workspace_root=WORKSPACE_ROOT)
