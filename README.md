...
| `MAX_TOOL_STEPS` | `24` | Tool rounds per request before Ankita answers with what it has (clamped 1-200). It also stops early if the same call repeats with identical arguments in three rounds |
| `MAX_TOOL_CALLS` | `60` | Total tool calls allowed per request, including calls batched in one model round (clamped 1-500) |
| `ANKITA_AGENT_DEBUG` | unset | Set to `1` for structured request, tool, duration, and termination metadata on stderr; argument values are not logged |
| `INPUT_COST_PER_MILLION` / `OUTPUT_COST_PER_MILLION` | unset | Enables `$` estimates in `/usage` |
| `PROVIDER` | `copilot` for CLI | Backend: `copilot`, `groq` (low latency; reuses `GROQ_API_KEY`, defaults to `openai/gpt-oss-120b`), or `kilo` — the [Kilo AI Gateway](https://kilo.ai/docs/gateway) with free, keyless models (`poolside/laguna-s-2.1:free` by default). Fresh desktop installs select Kilo |
