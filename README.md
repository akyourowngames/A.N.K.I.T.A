...
- **Calendar (Google)** — 5 model tools in `tools/calendar.py` (`zumba__calendar_today/search/create/brief/status`): `today/search/brief` answer from live events, `create` books, `status` checks connection. One global token at `~/.zumba/calendar_token.json` (file `0600`, dir `0700`, redacted previews); connect via `zumba calendar auth` (OAuth code wizard, `state` verified) or `zumba calendar token` (paste), `/cal auth <code>` in chat/Telegram, or web `POST /api/calendar/*`. Today uses your LOCAL day (`ZUMBA_TZ` or system tz). `brief` shows real drive estimates between back-to-back venues (via geo routing) + maps links + prep links; `daily` includes it when connected (fail-open, short timeout, never feeds errors to the LLM), honest “not connected” otherwise (never hallucinates); `ZUMBA_NO_CALENDAR=1` kill-switch. Telegram: `/cal [today|search|brief|status|auth|forget]` (code paste must match the strict `4/…` shape; `/cal forget yes` confirms). Web writes honor the kill-switch and optional `ZUMBA_CALENDAR_API_KEY` (`X-Zumba-Key`); like all `/api/*` they assume a localhost-bound server.
...
| `ZUMBA_TZ` | Local day tz for calendar (IANA, e.g. `Asia/Kolkata`) | system tz |
| `ZUMBA_CALENDAR_TIMEOUT` | Calendar HTTP timeout, 1–10s (`daily` uses 4s) | `10` |
| `ZUMBA_CALENDAR_API_KEY` | Shared secret for web calendar writes (`X-Zumba-Key`) | — (localhost assumed) |
