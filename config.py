...
NVIDIA_API_KEYS = [k for k in [NVIDIA_API_KEY] if k]
NVIDIA_BASE_URL = os.getenv("NVIDIA_BASE_URL", "https://integrate.api.nvidia.com/v1").rstrip("/")
NVIDIA_MODEL = os.getenv("NVIDIA_MODEL", "meta/llama-3.1-70b-instruct")
NVIDIA_FAST_MODEL = os.getenv("NVIDIA_FAST_MODEL", "meta/llama-3.1-8b-instruct")
NVIDIA_POWERFUL_MODEL = os.getenv("NVIDIA_POWERFUL_MODEL", "meta/llama-3.1-405b-instruct")
NVIDIA_VISION_MODEL = os.getenv("NVIDIA_VISION_MODEL", "nvidia/llama-3.1-nemotron-nano-vl-8b-v1")
NVIDIA_CODE_MODEL = os.getenv("NVIDIA_CODE_MODEL", "qwen/qwen2.5-coder-32b-instruct")
...
NVIDIA_EMBEDDING_BATCH_SIZE = int(os.getenv("NVIDIA_EMBEDDING_BATCH_SIZE", "64"))
NVIDIA_EMBEDDING_TYPE = os.getenv("NVIDIA_EMBEDDING_TYPE", "").strip()
NVIDIA_EMBEDDING_DIMENSIONS = os.getenv("NVIDIA_EMBEDDING_DIMENSIONS", "").strip()

DECISION_CACHE_ENABLED = os.getenv("DECISION_CACHE_ENABLED", "true").strip().lower() not in ("0", "false", "no", "off")
DECISION_CACHE_TTL = float(os.getenv("DECISION_CACHE_TTL", "300"))
DECISION_CACHE_SIZE = int(os.getenv("DECISION_CACHE_SIZE", "500"))
VECTOR_STORE_CACHE_ENABLED = os.getenv("VECTOR_STORE_CACHE_ENABLED", "true").strip().lower() not in ("0", "false", "no", "off")
SPECULATIVE_EXECUTION_ENABLED = os.getenv("SPECULATIVE_EXECUTION_ENABLED", "true").strip().lower() not in ("0", "false", "no", "off")
if NVIDIA_API_KEYS:
    GROQ_API_KEYS = NVIDIA_API_KEYS
...
VECTOR_INDEX_CHAT_HISTORY = os.getenv("VECTOR_INDEX_CHAT_HISTORY", "false").strip().lower() not in ("0", "false", "no", "off")
