...
def get_base_url() -> str:
    return (os.getenv(ENV_BASE_URL) or DEFAULT_BASE_URL).rstrip("/")


def get_api_key(require: bool = True) -> str:
    key = (os.getenv(ENV_API_KEY) or "").strip().strip('"').strip("'")
    if not key and require:
        raise RuntimeError(
            "KILO_API_KEY is not set. Get one at https://kilo.ai "
