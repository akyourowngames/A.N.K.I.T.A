...
from config import REALTIME_CHAT_ADDENDUM, GROQ_API_KEYS, NVIDIA_BASE_URL, NVIDIA_FAST_MODEL
...
class RealtimeGroqService(GroqService):
    ...
    def __init__(self, vector_store_service: VectorStoreService):
        ...
        self._fast_llm = (
            NvidiaClient(GROQ_API_KEYS[0], NVIDIA_BASE_URL, timeout=GROQ_REQUEST_TIMEOUT_FAST)
            if GROQ_API_KEYS
            else None
        )
    ...
    def get_response(self, question: str, chat_history: Optional[List[tuple]] = None, key_start_index: int = 0) -> str:
        ...
        try:
            ...
            t0 = time.perf_counter()
            response_content = self._invoke_llm(messages, key_start_index=key_start_index)
            logger.info("[TIMING] nvidia_api: %.3fs", time.perf_counter() - t0)
            logger.info(
